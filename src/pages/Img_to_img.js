import {
  ...
  export default function Img2img() {
    ...
    const generate = async (prompt) => {
      updateLoading(true);
    //   const result = await axios.get(`http://b321-35-240-222-226.ngrok.io/?prompt=${prompt}`);
    const result = await axios.get(`http://eefd-34-87-178-188.ngrok.io/?name=${name}`);
        ...
        Show {show ? 'Less' : 'More'}
          ...
          {loading ? (
            <Stack>
              ...
              <SkeletonText />
              <Text>Before</Text>
              <Image src={`http://ensaj.fertat.com/inscription/photos/${name}.jpg?673`} boxShadow=" 0 4px 8px 0 rgba(0, 0, 0, 0.2), 0 6px 20px 0 rgba(0, 0, 0, 0.19)" />
